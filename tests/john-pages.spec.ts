...
test('can authenticate a specific user', async ({ getUserPage }) => {
  ...
  await expect(
    customUserPage.getByRole('link', { name: 'Add Template' })
  ).toBeVisible({ timeout: 5000 });
  await expect(
    customUserPage.getByRole('link', { name: 'Browse Templates' })
  ).toBeVisible({ timeout: 5000 });

  await customUserPage.getByRole('link', { name: 'Add Template' }).click();
  await expect(
    customUserPage.getByRole('heading', { name: 'Add a Template' })
  ).toBeVisible({ timeout: 5000 });
